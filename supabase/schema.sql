...
create table if not exists public.reading_notes (
  ...
  body text,
  page_label text,
  current_page_snapshot integer not null default 0 check (current_page_snapshot >= 0),
  progress_percent_snapshot integer not null default 0 check (progress_percent_snapshot >= 0 and progress_percent_snapshot <= 100),
